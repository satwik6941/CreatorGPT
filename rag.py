...
def get_available_analysis_files():
    """Dynamically find available analysis files"""
    available_files = []
    
    # Find all batch files (flexible number)
    ...
    # Add other analysis files if they exist
    other_files = ['detailed_sentiment_results.txt', 'sentiment_analyzed_comments.csv']
    ...
    print(f"📁 Found {len(available_files)} analysis files: {available_files}")
...
# Initialize global variables for the indexes and retrievers
vector_index = None
keyword_index = None
...
def initialize_rag_system():
    """Initialize the RAG system with indexes and retrievers"""
    global vector_index, keyword_index, hybrid_retriever, hybrid_query_engine
    
    storage_dir = "storage"
    
    # Get available analysis files
    available_files = get_available_analysis_files()
    ...
    try:
        ...
        documents = SimpleDirectoryReader(input_files=available_files).load_data()
...
async def search_documents_with_context(query: str) -> str:
    ...
    global conversation_history, hybrid_query_engine
    
    if hybrid_query_engine is None:
        return "Error: RAG system not initialized. Please run initialize_rag_system() first."
