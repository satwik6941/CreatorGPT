...
const AdvancedDashboard = () => {
        ...
        <div className="bg-dark-surface border border-electric-blue/30 rounded-lg p-3 shadow-lg">
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} style={{ color: entry.color }}>
              {entry.dataKey}: {entry.value}{typeof entry.value === 'number' && entry.value < 10 ? '%' : ''}
            </p>
          ))}
        </div>
      );
  ...
  const getEngagementQuality = () => {
    if (!chartData?.average_sentiment) return '🔴 Low Confidence';
    if (chartData.average_sentiment > 60) return '🟢 High Confidence';
    if (chartData.average_sentiment > 40) return '🟡 Medium Confidence';
    ...
    <div className="min-h-screen bg-deep-black text-white">
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <Tabs defaultValue="charts" className="space-y-6">
          <TabsList className="grid w-full grid-cols-4 bg-dark-surface border border-electric-blue/20">
            <TabsTrigger value="charts" className="data-[state=active]:bg-electric-blue data-[state=active]:text-black">
              📊 Charts
            </TabsTrigger>
            <TabsTrigger value="keywords" className="data-[state=active]:bg-electric-blue data-[state=active]:text-black">
              🔤 Keywords
            </TabsTrigger>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="text-center p-3 bg-gradient-to-r from-green-900/20 to-green-700/20 rounded-lg border border-green-600/30">
                        <div className="text-green-400 text-2xl font-bold">
                          {chartData?.sentiment_distribution?.find(item => item.name === 'Positive')?.value?.toFixed(1) || 0}%
                        </div>
                      ...
                      <div className="text-center p-3 bg-gradient-to-r from-red-900/20 to-red-700/20 rounded-lg border border-red-600/30">
                        <div className="text-red-400 text-2xl font-bold">
                          {chartData?.sentiment_distribution?.find(item => item.name === 'Negative')?.value?.toFixed(1) || 0}%
                        </div>
          ...
          <TabsContent value="keywords" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="glass-effect border-electric-blue/20">
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={chartData?.word_frequency_positive || []}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                      <XAxis 
                        dataKey="word" 
                        tick={{ fill: '#9CA3AF', fontSize: 12 }}
              ...
              <Card className="glass-effect border-electric-blue/20">
                <CardHeader>
                  <CardTitle className="flex items-center space-x-2 text-red-400">
                    ...
                    <span>Top Negative Keywords</span>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={chartData?.word_frequency_negative || []}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                      <XAxis 
                        dataKey="word" 
                        tick={{ fill: '#9CA3AF', fontSize: 12 }}
              ...
              <Card className="glass-effect border-electric-blue/20 lg:col-span-2">
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={chartData?.confidence_distribution || []}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                      <XAxis 
                        dataKey="name" 
                        tick={{ fill: '#9CA3AF', fontSize: 12 }}
